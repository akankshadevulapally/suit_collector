...
timesToSample =  30   # sampling 4 times 
...
# Cycliic Learning Rate - Custom Implementations

max_lr = 0.0004
min_lr = max_lr / 6
lr_step = int(update_target_network * timesToSample / 3)
lr_inc = 1
lr_delta = (max_lr-min_lr) / lr_step
prev_lr = 1e-06 # Warming up is important
print(min_lr,max_lr,lr_step, lr_delta, prev_lr)
def getNextclr():
    global lr_inc 
    global lr_delta 
    global min_lr
...
#optimizer = keras.optimizers.Adam(learning_rate=0.00015625, clipnorm=1.0)
optimizer = keras.optimizers.Adam(learning_rate=prev_lr, clipnorm=1.0)
        ...
        for i in range(tn):
            ...
            for _ in range(timesToSample):
                ...
                if(len(replay_buffer_action) > min_memory_size):
                    ...
                    K.set_value(optimizer.learning_rate, getNextclr())
